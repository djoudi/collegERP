<?php

/**
 * Baselesson_fields
 * 
 * This class has been auto-generated by the Doctrine ORM Framework
 * 
 * @property integer $lesson_id
 * @property integer $field_id
 * @property object $value
 * @property lesson $lesson
 * @property fields $fields
 * 
 * @method integer       getLessonId()  Returns the current record's "lesson_id" value
 * @method integer       getFieldId()   Returns the current record's "field_id" value
 * @method object        getValue()     Returns the current record's "value" value
 * @method lesson        getLesson()    Returns the current record's "lesson" value
 * @method fields        getFields()    Returns the current record's "fields" value
 * @method lesson_fields setLessonId()  Sets the current record's "lesson_id" value
 * @method lesson_fields setFieldId()   Sets the current record's "field_id" value
 * @method lesson_fields setValue()     Sets the current record's "value" value
 * @method lesson_fields setLesson()    Sets the current record's "lesson" value
 * @method lesson_fields setFields()    Sets the current record's "fields" value
 * 
 * @package    ubeo
 * @subpackage model
 * @author     Your name here
 * @version    SVN: $Id: Builder.php 7490 2010-03-29 19:53:27Z jwage $
 */
abstract class Baselesson_fields extends sfDoctrineRecord
{
    public function setTableDefinition()
    {
        $this->setTableName('lesson_fields');
        $this->hasColumn('lesson_id', 'integer', null, array(
             'type' => 'integer',
             'notnull' => true,
             ));
        $this->hasColumn('field_id', 'integer', null, array(
             'type' => 'integer',
             'notnull' => true,
             ));
        $this->hasColumn('value', 'object', null, array(
             'type' => 'object',
             'notnull' => true,
             ));
    }

    public function setUp()
    {
        parent::setUp();
        $this->hasOne('lesson', array(
             'local' => 'lesson_id',
             'foreign' => 'id',
             'onDelete' => 'CASCADE'));

        $this->hasOne('fields', array(
             'local' => 'field_id',
             'foreign' => 'id',
             'onDelete' => 'CASCADE'));

        $timestampable0 = new Doctrine_Template_Timestampable();
        $this->actAs($timestampable0);
    }
}